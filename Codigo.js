...
function loginUsuario(username, password) {
  try {
    ...
    const userColIndex = headers.indexOf("Username") !== -1 ? headers.indexOf("Username") : headers.indexOf("Username");
    ...
    for (let i = 1; i < usersData.length; i++) {
      ...
      const dbEmail = headers.indexOf("Username") !== -1 ? (usersData[i][headers.indexOf("Username")] || "").toString().toLowerCase().trim() : "";
...
function obtenerDatosSocio(email) {
  try {
    ...
    const emailColIndex = userHeaders.indexOf("Username");
    ...
    let pagosEmailColIndex = pagosHeaders.indexOf("Username");
...
function obtenerDatosAdmin() {
  try {
    ...
    const userEmailColIndex = userHeaders.indexOf("Username");
    ...
    let emailColIndex = pagosHeaders.indexOf("Username");
    ...
    rowsPagos.forEach(row => {
      ...
      const socio = listaSocios.find(s => s.Username.toLowerCase() === email);
...
function registrarSocioNuevo(socioObj, userEmail) {
  try {
    ...
    const email = (socioObj.Username || socioObj.Username || "").trim().toLowerCase();
    ...
    const emailColIdx = headers.indexOf("Username");
    ...
    const rowArray = [];
    headers.forEach(h => {
      if (h === "Username") rowArray.push(email);
...
function generarPagoMercadoPago(paymentId) {
  try {
    ...
    let emailColIndex = headers.indexOf("Username");
...
function registrarPagoTransferenciaComprobante(paymentId, email, amount, month, paymentMethod) {
  try {
    ...
    const pEmailCol = pHeaders.indexOf("Username");
...
function conciliarPagoTransferenciaAutomatico(paymentId, email, amount, month, paymentMethod, base64Receipt, transactionId, ocrAmount, ocrText) {
  try {
    ...
    if (sheetUsers) {
      ...
      const uEmailCol = uHeaders.indexOf("Username");
...
function solicitarRevisionTransferencia(paymentId, email, amount, month, paymentMethod, base64Receipt, failReason) {
  try {
    ...
    const pEmailCol = headers.indexOf("Username");
...
function registrarSocioPublico(socioObj) {
  try {
    ...
    headers.forEach(header => {
      if (header === "Username") newRow.push(email);
...
function inicializarBaseDatos() {
  ...
  sheetAdmins.clear();
  sheetAdmins.appendRow(["Username"]);
  ...
  sheetUsers.clear();
  sheetUsers.appendRow(["Username","Role","Name","Phone","Category","DNI","BirthDate","Age","JoinDate","BloodType","MedicalFit","ObraSocial","EmergencyContact","EmergencyPhone","ParentName","ParentPhone","Notes","Photo","Username","Password"]);
  ...
  sheetPagos.clear();
  sheetPagos.appendRow(["Payment_ID","Username","Month","Amount","Status","MP_Link","Collected_By","Collected_At"]);
...
function verificarYGenerarCuotasMensuales() {
    ...
    const userEmailIdx = userHeaders.indexOf("Username");
    ...
    const pEmailCol = pagosHeaders.indexOf("Username");
